...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 21 23:06:18 2025
...
if pos_data is not None:
    ...
    # 绘制完整的XZ平面轨迹图
    # 用颜色表示时间变化（完整轨迹）
    scatter = plt.scatter(x, z, c=times, cmap='viridis', s=10, alpha=0.5, label='Full Trajectory')
    
    # 连接完整轨迹线
    plt.plot(x, z, 'gray', linewidth=0.5, alpha=0.3)
    ...
    if target_indices:
        ...
        # 用红色突出显示目标时间段轨迹
        plt.scatter(target_x, target_z, c='red', s=25, alpha=0.8, 
                   edgecolors='darkred', linewidth=1, label=f'05:00-06:00 UTC ({len(target_indices)} points)')
        
        # 连接目标时间段轨迹线
        plt.plot(target_x, target_z, 'r-', linewidth=2, alpha=0.8)
        
        # 标记目标时间段的起点和终点
        plt.plot(target_x[0], target_z[0], 'go', markersize=15, 
                markeredgecolor='white', markeredgewidth=2, label='05:00 UTC Start')
        plt.plot(target_x[-1], target_z[-1], 'bo', markersize=15, 
    ...
    # 标记完整轨迹的起点和终点
    plt.plot(x[0], z[0], 'k^', markersize=15, label='Full Trajectory Start', 
            markeredgecolor='white', markeredgewidth=2)
    plt.plot(x[-1], z[-1], 'kv', markersize=15, label='Full Trajectory End',
            markeredgecolor='white', markeredgewidth=2)
    
    # 添加地球（半径为1 Re）- 注意：在XZ平面中地球投影仍然是圆形
    ...
    plt.title('THEMIS-D Trajectory in XZ Plane (GSM Coordinates)\n' + 
    ...
    if target_indices:
        ...
        text_str += f"Start: X={target_x[0]:.2f} Re, Z={target_z[0]:.2f} Re\n"
        text_str += f"End: X={target_x[-1]:.2f} Re, Z={target_z[-1]:.2f} Re\n"
        text_str += f"Displacement: {np.sqrt((target_x[-1]-target_x[0])**2 + (target_z[-1]-target_z[0])**2):.2f} Re"
    ...
    # 显示基本统计信息 - 修改为XZ平面
    print("=" * 60)
    print("THEMIS-D TRAJECTORY ANALYSIS (XZ Plane)")
    print("=" * 60)
    ...
    if target_indices:
        ...
        print(f"Average Position: X={target_x.mean():.2f}, Z={target_z.mean():.2f} Re")
        
        # 计算到XZ平面中心的距离（地球中心在(0,0)）
        target_distances = np.sqrt(target_x**2 + target_z**2)
        print(f"Distance from Earth Center in XZ plane:")
        print(f"  Min: {target_distances.min():.2f} Re")
        print(f"  Max: {target_distances.max():.2f} Re")
        print(f"  Average: {target_distances.mean():.2f} Re")
        
        # 计算位移
        target_displacement = np.sqrt((target_x[-1] - target_x[0])**2 + (target_z[-1] - target_z[0])**2)
    ...
    # 添加一些关于XZ平面的额外信息
    print("\nXZ Plane Information:")
    print("- In GSM coordinates, X points from the Earth to Sun (sunward)")
    print("- Z points from the Earth to north (northward)")
    print("- Earth's projection in XZ plane is a circle centered at (0,0) with radius 1 Re")
